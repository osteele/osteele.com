...
function $H(data) {
    return {
        each: function(fn) {
            return Hash.each(data, fn)
        },
        keys: function() {
            return Hash.keys(data)
        },
        merge: function(other) {
            return Hash.merge(data, other)
        },
        toQueryString: function() {
            return Hash.toQueryString(data)
        },
        values: function() {
            return Hash.values(data)
        },
        compact: function() {
            return Hash.compact(data)
        },
        items: function() {
            return Hash.items(data)
        }
    };
}
...
Hash.toQueryString = function(hash) {
    ...
    for (name in hash) {
        var value = hash[name];
        typeof value == 'function' || words.push([name, '=', LzBrowser.urlEscape(value)].join(''));
    }
    words.sort();
    return words.join('&');
}
...
String.prototype.truncate = function(length, ellipsis) {
    return ( this.length <= length ? string : string.slice(0, length) + ellipsis) ;
}
