...
class hn_captcha
{
	...
	//
		...
		function hn_captcha($config,$secure=TRUE)
		{
			...
			$this->messages = array(
				'de'=>array(
							'msg1'=>'Du muﾟt die <b>'.$this->chars.' Zeichen</b> im Bild, (Zahlen&nbsp;von&nbsp;<b>0&nbsp;-&nbsp;9</b> und Buchstaben&nbsp;von&nbsp;<b>A&nbsp;-&nbsp;F</b>),<br>in das Feld eintragen und das Formular abschicken um den Download zu starten.',
	...
	//
		...
		function display_form()
		{
			...
			$s  = '<div id="captcha">';
			$s .= '<form class="captcha" name="captcha1" action="' . $_SERVER['PHP_SELF'] . '" method="POST">'."\n";
			$s .= '<input type="hidden" name="hncaptcha" value="'.$try.'">'."\n";
			$s .= '<input type="hidden" name="c_id" value ="'.$c_id.'" />'."\n";
			$s .= '<input type="hidden" name="c_author" value ="'.$c_author.'" />'."\n";
	...
	//
		...
		function make_captcha()
		{
			...
			if($this->nb_noise > 0)
			{
				...
				for($i=0; $i < $this->nb_noise; $i++)
				{
					...
					//### DdV: fix for Debian Linux?
					imagestring($image, $this->change_TTF(), $x, $y, $text, $color);
		...
		//### DdV: Debian fix:
			return 3;
		//	return imageloadfont($this->TTF_file);
			/////
