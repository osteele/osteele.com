	...
	if(theRules == "cssRules")
	{
		for (var j = 0; j < document.styleSheets.length; j++) {
			...
			for (var i = 0; i < document.styleSheets[j].cssRules.length; i++) {
				var myclass = document.styleSheets[j].cssRules[i].selectorText.toLowerCase();
	...
	else
	{
		for (var j = 0; j < document.styleSheets.length; j++) {
			...
			for (var i = 0; i < document.styleSheets[j].rules.length; i++) {
				var myclass = document.styleSheets[j].rules[i].selectorText.toLowerCase();
		///Convert Safari #ids into proper format
				newclass = myclass.replace(/\*\[id\"([^\"]*)\"\]/g, "\#$1");
				if (getCookie(newclass)) {
					document.cookie = newclass + "="+""+" "+expires+path;
...
function cookiecss () {
	...
	var theRules = String(checkBrowser());

	if(theRules == "cssRules")
	{
		for (var k = 0; k < document.styleSheets.length; k++) {
	
			for (var i = 0; i < document.styleSheets[k].cssRules.length; i++) {
				var myclass = document.styleSheets[k].cssRules[i].selectorText.toLowerCase();
				///Convert Safari #ids into proper format
				newclass = myclass.replace(/\*\[id\"([^\"]*)\"\]/g, "\#$1");
				if (getCookie(newclass)) {
					...
					for (var j = 0; j < firstsplit.length; j++) {
						...
						if (secondsplit[0] != "") {
							...
							document.styleSheets[k].cssRules[i].style[secondsplit[0]] = secondsplit[1];
	...
	else
	{
		for (var k = 0; k < document.styleSheets.length; k++) {
			...
			for (var i = 0; i < document.styleSheets[k].rules.length; i++) {
				var myclass = document.styleSheets[k].rules[i].selectorText.toLowerCase();
				///Convert Safari #ids into proper format
				newclass = myclass.replace(/\*\[id\"([^\"]*)\"\]/g, "\#$1");
				if (getCookie(newclass)) {
					var cvalue = getCookie(newclass);
					...
					for (var j = 0; j < firstsplit.length; j++) {
						...
						if (secondsplit[0] != "") {
							///Update CSS from Cookie data
							document.styleSheets[k].rules[i].style[secondsplit[0]] = secondsplit[1];
							if (document.getElementById(secondsplit[2])) {
								hex = secondsplit[1].replace(/\#/,'');
								document.getElementById(secondsplit[2]).value = hex;
							}
