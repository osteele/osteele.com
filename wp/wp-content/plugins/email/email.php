	...
	if (not_spamming()) {
		if(not_password_protected()) {
			...
			if($email_image_verify) {
				...
				$output .= '<img src="'.get_option('siteurl').'/wp-content/plugins/email/email-image-verify.php" width="55" height="15" style="margin: 0; padding: 0 10px 0 0; float: left;" alt="'.__('E-Mail Image Verification', 'wp-email').'" /><input type="text" size="5" maxlength="5" id="imageverify" name="imageverify" class="Forms" />'."\n";
				$output .= '</p>'."\n";
			}
			$output .= '<p><input type="button" value="'.__('     Mail It!     ', 'wp-email').'" id="wp-email-submit" class="buttonem" onclick="email_form();" onkeypress="email_form();" /></p>'."\n";
			$output .= '</form>'."\n";
		} else { 
...
function create_email_table() {
	...
	add_option('email_template_subtitle', '<p style="text-align: left; text-transform:uppercase; padding-top: 20px;">'.__('Email a copy of <strong>\'%EMAIL_POST_TITLE%\'</strong> to a friend', 'wp-email').'</p>', 'Template For E-Mail Page SubTitle');
