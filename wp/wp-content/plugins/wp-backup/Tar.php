...
// +----------------------------------------------------------------------+
//
// $Id: Tar.php,v 1.1 2004/01/09 07:58:12 wolf Exp $
...
class Archive_Tar extends PEAR
{
    ...
    * @access public
    */
    function create($p_filelist, $p_exclude)
    {
        return $this->createModify($p_filelist, '', '', $p_exclude);
    }
    // }}}
    ...
    * @see addModify()
    */
    function createModify($p_filelist, $p_add_dir, $p_remove_dir='', $p_exclude)
    {
        $v_result = true;
        ...
        if ($p_filelist != '') {
            ...
            $v_result = $this->_addList($v_list, $p_add_dir, $p_remove_dir, $p_exclude);
        }
    ...
    */
    ...
    // {{{ _addList()
    function _addList($p_list, $p_add_dir, $p_remove_dir, $p_exclude)
    {
      $v_result=true;
      $v_header = array();
      ...
      for ($j=0; ($j<count($p_list)) && ($v_result); $j++) {
        ...
        if ($v_filename == $this->_tarname)
            continue;
		for ($y=0; ($y<count($p_exclude)) && ($v_result); $y++) {
			if (preg_match('/'.$p_exclude[$y].'/i', $v_filename))
	            continue;	
		}
        ...
        if (@is_dir($v_filename)) {
			for ($r=0; ($r<count($p_exclude)) && ($v_result); $r++) {
				if (preg_match('/'.$p_exclude[$r].'/i', $v_filename))
		            continue;	
			}        	
            ...
            while (false !== ($p_hitem = readdir($p_hdir))) {
                ...
                $v_result = $this->_addList($p_temp_list, $p_add_dir, $p_remove_dir, $p_exclude);
            }
    ...
    */
    ...
    function _append($p_filelist, $p_add_dir='', $p_remove_dir='')
    {
        ...
        if ($this->_addList($p_filelist, $p_add_dir, $p_remove_dir, $p_exclude))
           $this->_writeFooter();
