...
// abstract (override as needed):
	...
	function sk2_plugin()
	{
		...
		if (!$this->is_filter() && !$this->is_treatment())
			$this->log_msg ("Plugin <i>" . $this->name . "</i> is registered with both <code>filter</code> and <code>treatment</code> set to false. It probably won't do anything: please contact plugin author" . (empty($this->author_email) ? "." : (": <a href=\"mailto:" . $this->author_email . "\">" . $this->author_email ."</a>.")) , 5);
	...
	{ // override this to do your own filtering
		log_msg ("Default filter (no action) called for plugin: " . $name, 3, $cmt_object->ID);
	}
	
	function treat_this(&$cmt_object)
	{ // override this to do your own treatment
		log_msg ("Default treatment (no action) called for plugin: " . $name, 3, $cmt_object->ID);
	}
...
// 	private
	...
	function output_plugin_UI ($output_dls = true) // you can override this function if you  *really* know what you are doing
	{
		...
		if (! isset($this->weight_levels["0.0"]) && ! isset($this->weight_levels["0"]))
			$this->weight_levels["0"] = "Disabled";
		ksort($this->weight_levels);	

		echo " <strong>$this->name</strong> - Strength: ";
		...
		if (! empty($this->author))
		{
			...
			if (empty($this->author_url))
				...
				echo "<a href=\"$this->author_url\" title=\"". __("Visit author's webpage") . "\">" . $this->author . "</a>";
		}
	...
	{
		log_msg ("Default Second Hand function (empty) called for plugin: " . $this->name, 7, $comment_ID);
		
		echo "This is an empty placeholder...";
	}
