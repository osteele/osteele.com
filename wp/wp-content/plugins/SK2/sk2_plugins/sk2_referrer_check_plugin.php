...
class sk2_referrer_check_plugin extends sk2_plugin
{
	...
	var $author = "";
	var $plugin_help_url = "http://wp-plugins.net/wiki/index.php/SK2_ReferrerCheck_Plugin";
	var $filter = true;
	...
	function filter_this(&$cmt_object)
	{
		...
		{
			$log = "Trackback Source Site (<em>". $cmt_object->author_url['href'] ."</em>) unreachable.";
		...
		{
			$log = "Trackback Source Site (<em>". $cmt_object->author_url['href'] ."</em>) <strong>does</strong> contain Blog URL domain (<em>$this_server</em>).";
		...
		{
			$log = "Trackback Source Site (<em>". $cmt_object->author_url['href'] ."</em>) does <strong>not</strong> contain Blog URL domain (<em>$this_server</em>).";
	...
	function output_plugin_UI()
	{
		...
		if(! function_exists("curl_init") && ! ini_get('allow_url_fopen') )
			echo "<dt><strong><p style=\"color:red;\">Both <code>allow_url_fopen</code> and <code>CURL</code> are disabled on this PHP install: TB Referrer Check plugin cannot run. You should disable it.</p></strong></dt>";
		echo "</dl>";
	}
}
