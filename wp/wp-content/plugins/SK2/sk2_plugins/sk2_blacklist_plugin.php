...
class sk2_blacklist_plugin extends sk2_plugin
{
	var $name = "Blacklist";
	var $author = "";
	var $plugin_help_url = "http://wp-plugins.net/wiki/index.php/SK2_Blacklist_Plugin";
	var $filter = true;
	var $treatment = true;
	var $description = "";
	...
	//	echo "</pre>";
		...
		$this->log_msg("Running sk2_blacklist plugin on comment ID: " . $cmt_object->ID, 0);
	...
	function treat_this(&$cmt_obj)
	{
		...
		if (($auto_blacklist > 0) ||  ($auto_whitelist > 0))
		{
			...
			if ($cmt_obj->is_post_proc() && ($score > 30))
			{
				...
				{
					$this->log_msg("Downgrading ${opposite}list entries.", 5);			
	...
	function update_SQL_schema($cur_version)
	{
		...
		foreach(array("typepad.com", "blogspot.com", "livejournal.com", "xanga.com") as $grey_domain)
		{
			...
			$this->log_msg("Added default domain_grey entry: $grey_domain", 6);
		}
