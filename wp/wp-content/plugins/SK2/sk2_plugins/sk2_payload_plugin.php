...
class sk2_payload_plugin extends sk2_plugin
{
	...
	var $author = "";
	var $plugin_help_url = "http://wp-plugins.net/wiki/index.php/SK2_Payload_Plugin";
	...
	function form_insert($post_ID)
	{
		...
		if (empty ($seed))
		{
			...
			$this->log_msg("Resetting secret seed to: $seed.", 5);
		}
		$time = time();
	...
	{					
		...
		{
			$log = "Cannot check encrypted payload in post_proc mode.";
			$this->log_msg($log, 4);
			return;
		}	
		...
		{
			$log = "Encrypted Payload missing from form.";
			$karma_diff = -20;
			$this->log_msg($log, 1);
		}
		elseif($cmt_object->post_ID != $_REQUEST['comment_post_ID'])
		{
			$log = "Error: Submitted Post_ID variable (" . $_REQUEST['comment_post_ID'] .") not matching ours (" . $cmt_object->post_ID .  ").";
			$this->log_msg($log, 9);
			$karma_diff = -8;
		}
		else
		{
			...
			{
				$log = "Encrypted payload valid: IP matching.";
				$karma_diff = 0.50;
			}
			else
			{
				$log = "Encrypted payload valid: IP <strong>not</strong> matching.";
				$karma_diff = - 2.5;
			}
		}
