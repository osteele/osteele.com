...
class sk2_rbl_plugin extends sk2_plugin
{
	...
	var $plugin_help_url = "http://wp-plugins.net/wiki/index.php/SK2_RBL_Plugin";
  ...
  */
  function rbl_lookup($rbl_host)
  {
    ...
    if (strpos($rbl_host, '/') !== false)
    {
    	...
    	{
	    	$this->log_msg("$rbl_host has a slash in it; falling back to checkdnsrr() :(", 0);
    ...
    $this->log_msg("rbl_lookup: lookup for $rbl_host returned $get_host", 2);
    if ($get_host
      && ($get_host != $rbl_host)
			...
			foreach ($rbl_list as $rbl_row)
			{
				...
				foreach($check_list as $this_addr)
				{
					...
					if (($get_host = $this->rbl_lookup($rbl_host))
						&& ($get_host != ''))
					{
						$this->hit_karma($cmt_object, 5, "IP ($cmt_object->author_ip) matched RBL server $rbl_server (returned: <em>$get_host</em>)");
						$used_ids[] = $rbl_row->id;
					}
					else //DEBUG
						$this->log_msg("IP ($cmt_object->author_ip) not listed by RBL $rbl_server (response: $get_host)", 0);
		...
		if ($rbl_list = $sk2_blacklist->get_list('rbl_server_uri'))
		{
			foreach ($rbl_list as $rbl_row)
			{
				...
				if (($get_host = $this->rbl_lookup($rbl_host))
					&& ($get_host != ''))
				{
					$this->hit_karma($cmt_object, 5, "Author's URL ($author_url) appears in RBL at $rbl_server (returned: <em>$get_host</em>)");
					$used_ids[] = $rbl_row->id;
				}
				else
					$this->log_msg("Author's URL ($author_url) does not appear in RBL $rbl_server",3);				
				...
				foreach ($found_uris as $url_info)
				{
					...
					if (($get_host = $this->rbl_lookup($rbl_host))
						&& ($get_host != ''))
					{
						$this->hit_karma($cmt_object, 5, "URI (" . $url_info['url']. ") matched RBL server $rbl_server (returned: <em>$get_host</em>)");
						$used_ids[] = $rbl_row->id;
					}
					else 
					{
						//debug
						$this->log_msg("URI $rbl_host not listed by RBL $rbl_server", 3);
					}
				}
				if ($cmt_object->karma < -10)
					break;
	...
	function update_SQL_schema($cur_version)
	{
		...
		foreach (array("opm.blitzed.org", "bl.blbl.org") as $rbl)
		{
			...
			$this->log_msg("Added default IP RBL server entry: $rbl", 4);
		}
		...
		if ((($cur_version == 1) && ($this->plugin_version == 2)) || ($cur_version == 0))
		{
			...
			foreach (array("uri-bl.blbl.org") as $rbl)
			{
				...
				$this->log_msg("Added default URI RBL server entry: $rbl", 4);
			}
		}
	...
	function treat_this($cmt_object)
	{
		...
		{
				$this->log_msg("Submittng to URL using CURL", 3);
				...
				foreach ($found_uris as $url_info)
	    	{
				...
				if (!empty($postinfo))
				{
					...
					$this->log_msg("Submitted " . sizeof($postinfo) . " total entries to RBL using POST; got result $result", 4);
				}
		...
		$this->log_msg("Submitting to URL using GET", 3);
  	...
  	if (!empty($ip)) 
  	{
  		$url = sprintf($this->get_option_value('submit_ip_target'), $ip);
			$this->log_msg("Opening URL: $url", 0);
			$result = sk2_url_fopen($url);
  		$this->log_msg("Adding IP $ip to RBL returned $result", 2);
  		$ipcount++;
  	}
		...
		if (!empty($cmt_object->author_url['url']))
		{
			...
			$this->log_msg("Opening URL: $url", 0);
			$result = sk2_url_fopen($url);
			$this->log_msg("Adding author URI " . $cmt_object->author_url['url'] . " to RBL returned $result.", 2);
			$uricount++;
		}
  	...
  	foreach ($found_uris as $url_info)
  	{
  		if (!in_array($url_info['url'], $done_uris))
  		{
  			$done_uris[] = $url_info['url'];
				...
				$this->log_msg("Opening URL: $url", 0);
				$result = sk2_url_fopen($url);	
    		$this->log_msg("Adding URI " . $url_info['url'] . " to RBL returned $result.", 2);
    		$uricount++;
    	}
		}
		
		$this->log_msg("Submitted $ipcount IP(s) and $uricount URI(s) to RBL using GET", 4);
	}
}
