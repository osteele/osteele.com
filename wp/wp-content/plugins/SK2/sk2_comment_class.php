...
// Diagnostic data:
	...
	function sk2_comment($comment_id, $post_proc = false, $comment_sk_info = 0)
	{
		...
		{
			$this->log_msg("sk2_comment: Cannot fetch comment record from table.", 9, true);
			return false;
		}	
		$this->ID = $comment_id;
	...
	function modify_karma($karma_diff, $plugin_name, $reason = "")
	{
		...
		$this->karma_cmts[] = array("ts" => time(), "hit" => $karma_diff, "plugin" => $plugin_name, "reason" => $reason);
	}
	...
	function set_DB_status($new_status, $plugin = "", $update_karma = false, $id = 0)
	{
		...
		if (! mysql_error())
		{
			$this->log_msg("Successfully updated comment entry ID: $id to status: $new_status.", 4, false, $plugin);
			$this->approved = $wp_status;
			return true;
		}
		else
		{
			$this->log_msg("Error: cannot update comment entry ID: $id to status: $new_status.", 7, true, $plugin);
			return false;
		}
	}
