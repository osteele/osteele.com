...
class sk2_blacklist
{
	...
	function add_entry($type, $value, $score = 100, $user_reviewed = "no", $added_by = "unknown", $trust = 100)
	{
		...
		{
			$this->log_msg("Greylist match. Skipping blacklist entry insertion: <em>$type</em> - <em> $value</em>.", 7);
			return 0;
		}
		...
		if (empty($value))
		{
			$this->log_msg("Cannot add blacklist entry. Please fill in a value.", 7);
			return false;
		}
		elseif ($wpdb->get_var("SELECT COUNT(*) FROM `". sk2_kBlacklistTable . "` WHERE `type`='$type' AND `value`='" . sk2_escape_string($value) . "' LIMIT 1"))
		{
			$this->log_msg("Skipping duplicate blacklist entry: <em>$type</em> - <em> $value</em>.", 7);
		}
		else
		{
			if ($wpdb->query("INSERT INTO  `". sk2_kBlacklistTable . "` SET `type`='$type',  `value`='" . sk2_escape_string($value) . "', `added` = NOW(), `last_used` = NOW(), `score` = $score, `trust` = $trust, `user_reviewed` = '$user_reviewed', `added_by` = '$added_by' "))
					$this->log_msg("Successfully inserted blacklist entry: <em>$type</em> - <em>$value</em>.", 3);
			else
					$this->log_msg("Failed to insert blacklist entry: <em>$type</em> - <em>$value</em>.", 8, true);
		}
	...
	function auto_add($type, $value, $score = 100, $user_reviewed = "no", $added_by = "unknown", $trust = 100)
	{
		...
		elseif	 (($type == "domain_black" || $type == "domain_white")
			&& ($grey_rslt = $wpdb->get_results("SELECT * FROM `" . sk2_kBlacklistTable . "` WHERE `type` = 'domain_grey' AND `value` = '$value'")))
		{
			$this->log_msg("Greylist match. Skipping blacklist entry insertion: <em>$type</em> - <em> $value</em>.", 6);
			return;
		}
		...
		{
			$query = "INSERT INTO `". sk2_kBlacklistTable . "` SET `type`='$type',  `value`='" . sk2_escape_string($value) . "', `added` = NOW(), `last_used` = NOW(), `trust` = $trust, `user_reviewed` = '$user_reviewed', `added_by` = '$added_by', ";
			$query_where = "";
			$old_score = 0;
		}
		...
		if (! mysql_error())
		{
			$this->log_msg("Successfully inserted/updated blacklist entry: <em>$type</em> - <em>$value</em>. Current score: $score", 3);
			return true;
		}
		else
			$this->log_msg("Failed to insert blacklist entry: <em>$type</em> - <em>$value</em>.", 8, true);
	...
	function match_entries($match_type, $match_value, $strict = true, $min_score = 0)
	{
		...
		if ($strict)
		 ...
		 switch ($match_type)
		 {
			...
			case 'regex_content_match':
				...
				if ($regex_recs = $wpdb->get_results("SELECT * FROM `" . sk2_kBlacklistTable . "` WHERE `type` = '${type}_white' OR `type` = '${type}_black'"))
					foreach($regex_recs as $regex_rec)
					{
						...
						if ($res === FALSE)
							$this->log_msg("Regex ID: $regex_rec->id (<code>$regex_rec->value</code>) appears to be an invalid regex string! Please fix it in the Blacklist control panel.", 7);
						elseif ($res)
							$query_where .= $regex_rec->id . ", ";
					}
				$query_where .= "-1)";
			...
			case 'ip_white':
				...
				{
					$query_where = "";
					$this->log_msg("Grey blacklist match: ignoring.", 6);					
				}
				else
		...
		else
		{
			...
			if (mysql_error())
			{
				$this->log_msg("Failed to query blacklist: <em>$match_type</em> - <em>$match_value</em>. Query: $query", 8, true);
				return false;
			}
	...
	function get_list($type, $limit = 0)
	{
		...
		if (mysql_error())
		{
			$this->log_msg("get_list: Failed to get blacklist entries of type <em>$type</em>. Query: $query", 8, true);
			return false;
		}
	...
	function increment_used ($ids)
	{
		...
		$wpdb->query($query);
		if (mysql_error())
			$this->log_msg("Failed to update blacklist used count.</br>Query: $query", 8, true);
	...
	function downgrade_entries ($ids)
	{
		...
		$wpdb->query($query);
		if (mysql_error())
			$this->log_msg("Failed to downgrade blacklist scores.</br> Query: $query", 8, true);
