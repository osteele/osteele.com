...
function sk2_get_file_list($path, $ext = ".php")
{
	...
	if (! is_dir($path))
	{
		$sk2_log->log_msg("Cannot get file list: '" . $path . "' is not a valid folder path", 9, 'sk2_get_file_list');
		return $files;
	}
	elseif (! is_readable($path))
	{
		$sk2_log->log_msg("Cannot get folder content: '" . $path . "'. Please make sure it is readable by everybody (chmod 755).", 9, 'sk2_get_file_list');
	}
...
function sk2_clean_up_sql_callback(&$str, $junk)
{
